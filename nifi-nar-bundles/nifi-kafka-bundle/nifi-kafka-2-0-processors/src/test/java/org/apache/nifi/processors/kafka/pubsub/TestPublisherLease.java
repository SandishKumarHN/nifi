...
public class TestPublisherLease {
    ...
    public void testPoisonOnException() throws IOException {
        ...
        try {
            lease.publish(flowFile,null, failureInputStream, messageKey, demarcatorBytes, topic);
    ...
    public void testPoisonOnFailure() throws IOException {
        ...
        lease.publish(flowFile, null,new ByteArrayInputStream(new byte[1]), messageKey, demarcatorBytes, topic);
    ...
    public void testAllDelimitedMessagesSent() throws IOException {
        ...
        lease.publish(flowFile, null, new ByteArrayInputStream(flowFileContent), messageKey, demarcatorBytes, topic);

        final byte[] flowFileContent2 = new byte[0];
        lease.publish(new MockFlowFile(2L),null, new ByteArrayInputStream(flowFileContent2), messageKey, demarcatorBytes, topic);

        final byte[] flowFileContent3 = "1234567890\n1234567890".getBytes(StandardCharsets.UTF_8); // no trailing new line
        lease.publish(new MockFlowFile(3L),null, new ByteArrayInputStream(flowFileContent3), messageKey, demarcatorBytes, topic);

        final byte[] flowFileContent4 = "\n\n\n".getBytes(StandardCharsets.UTF_8);
        lease.publish(new MockFlowFile(4L), null,new ByteArrayInputStream(flowFileContent4), messageKey, demarcatorBytes, topic);
    ...
    public void testRecordsSentToRecordWriterAndThenToProducer() throws IOException, SchemaNotFoundException, MalformedRecordException {
        ...
        lease.publish(flowFile, null, recordSet, writerFactory, schema, keyField, topic);
